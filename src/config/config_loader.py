...
class VISTConfig:
    ...
    def robot_shoulder_position(self):
        """机器人肩部位置 [x, y, z]（根据 arm_side 自动选择）"""
        arm_side = self.hardware_arm_side
        # 尝试从 shoulder_positions 字典中获取
        if 'shoulder_positions' in self._config.get('robot', {}):
            positions = self._config['robot']['shoulder_positions']
            if arm_side in positions:
                return np.array(positions[arm_side], dtype=np.float64)
        # 回退到默认的 shoulder_position
